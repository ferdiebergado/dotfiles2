...
handle_mime() {
    ...
    case "${mimetype}" in
        ...
        text/* | */xml)
            ...
            #source-highlight -q --outlang-def=esc.outlang --style-file=esc.style -i "${FILE_PATH}" || cat -- "${FILE_PATH}"
            exit 2
            ;;
        ...
        image/*)
            ...
            #exiftool "${FILE_PATH}"
            #~/.config/lf/draw_img.sh "${FILE_PATH}"
            img2sixel -- "${FILE_PATH}" && exit 1
            #exit 1
            ;;
